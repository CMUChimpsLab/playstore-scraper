...
pairCountDF.sort(["count"], ascending = False).to_csv("./stats/jialiuNumberOfAppsForPairs.csv", sep="\t")
...
permissionCountDF.sort(["count"], ascending = False).to_csv("./stats/jialiuNumberOfPairsForPermissions.csv", sep="\t")

purposeCountDF = df.groupby(["purpose"])["packageName"].count().reset_index()
purposeCountDF.columns = ["purpose", "count"]
purposeCountDF.sort(["count"], ascending = False).to_csv("./stats/jialiuNumberOfPairsForPurposes.csv", sep="\t")

responseDF = pd.read_csv("./cleanResponseWithPackageNameInCurrentDBFromJialiuAdjusted.csv", sep="\t")
...
responseCountDF.sort(["count"], ascending = False).to_csv("./stats/jialiuNumberOfResponsesPerHits.csv", sep="\t")
