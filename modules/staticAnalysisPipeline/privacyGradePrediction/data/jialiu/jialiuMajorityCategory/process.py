...
appNameToCateogryMap = df.groupby(["appname"])["Q2category"].agg(lambda x:x.value_counts().index[0])
#filter out entries where Q2category is not the mode 
cleanResponseDF = df[df["Q2category"] == appNameToCateogryMap[df["appname"]]].reset_index(drop=True)
