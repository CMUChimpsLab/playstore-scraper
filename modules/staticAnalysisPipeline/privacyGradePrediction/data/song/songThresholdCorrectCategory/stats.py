...
df = pd.read_csv("./comfortScorePerPairFromSong.csv", sep="\t")
pairCountDF = df.groupby(["permission", "purpose"])["packageName"].nunique().reset_index()
pairCountDF.columns = ["permission", "purpose", "count"]
pairCountDF.sort(["count"], ascending = False).to_csv("./stats/songNumberOfAppsForPairs.csv", sep="\t", index = False)
...
categoryCntFile = open("./stats/totalNumberOfHitsPerCateogry.csv", "w")
...
permissionCountDF.sort(["count"], ascending = False).to_csv("./stats/songNumberOfPairsForPermissions.csv", sep="\t", index = False)

purposeCountDF = df.groupby(["purpose"])["packageName"].count().reset_index()
purposeCountDF.columns = ["purpose", "count"]
purposeCountDF.sort(["count"], ascending = False).to_csv("./stats/songNumberOfPairsForPurposes.csv", sep="\t", index = False)

responseDF = pd.read_csv("./cleanResponseFromSong.csv", sep="\t")
...
responseCountDF.sort(["count"], ascending = False).to_csv("./stats/songNumberOfResponsesPerHits.csv", sep="\t", index = False)

#Get qualification score for every workerid
workeridList = responseDF["workerid"].unique()
workeridQualificationScoreFile = open("./stats/workeridQualificationScore.csv", "w")
...
for workerid in workeridList:
    try:
        ...
        print >> workeridQualificationScoreFile, "%s\tN/A"%workerid
    else:
