...
def fix(uuid, helper=None, client=None):
    ...
    permissionlist = db['Test_permissionlist'].find({'filename': uuid+'.apk'}, {'src': 1, 'is_external': 1, 'permission': 1, 'dest': 1, '_id': 0})
    l = [{'src': i['src'], 'is_external': i['is_external'], 'permission': i['permission'], 'dest': i['dest']} for i in permissionlist]
